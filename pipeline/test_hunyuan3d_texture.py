...
torch/trimesh and runs a real paint pipeline. It is excluded from pytest
collection (root conftest.py `collect_ignore_glob`) and all executable work
lives under the `if __name__ == "__main__":` guard below so importing this
module can never trigger a GPU run.
